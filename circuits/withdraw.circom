...
template CommitmentHasherBits() {
    // Commitment hash outputs:
    // - `hashBits`: Keccak output bits (preferred internal representation)
    // - `hash`:     Keccak output bytes (kept for compatibility with existing callers)
    ...
    // 32 + 32 + 8 + 4 = 76 bytes = 608 bits
    component keccak = Keccak(608, 256);
    var idx = 0;
    component nBits[32];
    for (var i = 0; i < 32; i++) {
    ...
    // NOTE: Bits2Num does not add boolean constraints; Keccak already constrains its outputs as bits.
...
template Withdraw(levels) {
    ...
    //      (commitment = keccak(nullifier || secret || amount || assetId)).
    ...
    // 1. Commitment
    component cHasher = CommitmentHasherBits();
    for(var i=0; i<32; i++) { cHasher.nullifier[i] <== nullifier[i]; cHasher.secret[i] <== secret[i]; }

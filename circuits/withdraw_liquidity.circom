...
// Liquidity withdraw circuit (share-based, pool-bound).
//
// Leaf/commitment (bytes):
//   keccak256(nullifier || secret || sharesLE8 || poolIdLE4)
//
// Public inputs:
//   - rootHi, rootLo
//   - nullifierHashHi, nullifierHashLo
...
template CommitmentHasherBytes() {
...
template WithdrawLiquidity(levels) {
    ...
    //      (commitment = keccak(nullifier || secret || shares || poolId)).
    ...
    // 1) Commitment
    component cHasher = CommitmentHasherBytes();
